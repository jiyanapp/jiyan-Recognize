                ...
                while (true) {
                    ...
                    switch (status) {
                        case StatusFirstFrame:   // 第一帧音频status = 0
                            ...
                            business.addProperty("accent", accent);//中文方言参数值
        ...
        if (resp != null) {
            ...
            if (resp.getData() != null) {
                ...
                if (resp.getData().getStatus() == 2) {
                    ...
                    //System.out.println("11最终识别结果 ==》" + decoder.toString());
...
/* 以上为固定部分(难以修改)，以下为可修改部分 */    
    
  //第一个参数是PCM文件的地址，第二个为方言参数(如四川话lmz)，第三个为识别的文本
    public static double recognize(String File, String Accent, String ans) throws Exception{
    	file = new String(File);                                                          //提交到讯飞云端进行识别
        ...
        WebSocket webSocket = client.newWebSocket(request, MyTry);                       //主线程等待至其他所有进程结束，获得识别结果result
        iatCountDown.await();
        System.out.println(Result);
        
        int len = ans.length(), same = 0;
        for(int i = 0; i < len; i++)                                                 //进行简单的海明距离比较
        {
        	if(ans.charAt(i) == Result.charAt(i)) same++;
        }
       
       double res = 100.0 * same / len;
       System.out.printf("识别分数为：%.2f", res);
       return res;                                                                     //res为识别分数
    }
    
    //测试部分
    public static void main(String[] args) throws Exception {    //第一个参数是PCM文件的地址，第二个为方言参数，第三个为识别的文本
